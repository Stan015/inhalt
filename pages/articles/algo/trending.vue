...
const fecthArticles = async () => {
  ...
  try {
    ...
    if (data) {
      articlesStore.fetchedTrendingArticles = data as Array<Article>;
    ...
    articlesStore.fetchTrendingError = error as Error;
  } finally {
...
<template>
  ...
  >
    ...
    <main class="block w-full h-full pb-6 text-primary dark:text-primary">
      <DiscoveryNavLinks current-page="trending" parent-page="articles" />
