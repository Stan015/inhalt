  ...
  <header class="w-full">
    ...
    >
      ...
      >
        ...
        <div
          v-if="showMenu"
          class="flex flex-col items-end justify-between py-4 gap-3 w-full min-h-[calc(100dvh-4rem)] text-primary dark:text-primary bg-black/[0.9] absolute z-40 top-[3.7rem] max-sm:top-[2.6rem]"
        >
          <nav class="flex flex-col gap-4 w-1/2 max-sm:w-[12rem] h-max">
            <NuxtLink
              to="/"
              :aria-current="currentRoute === '/' ? 'true' : 'false'"
            ...
            <NuxtLink
              to="/articles"
              :aria-current="currentRoute === '/articles' ? 'true' : 'false'"
              class="flex flex-shrink-0 items-center gap-3 w-[90%] bg-white border-b-2 border-b-white hover:border-b-accent transition-all px-4 py-[1.5px] rounded-l-lg relative after:content-[' '] after:w-2 after:h-full after:absolute after:right-[-10%] after:rounded-l-2xl after:bg-white after:pointer-events-none aria-[current=true]:after:bg-accent"
              @click="
                () => {
                  toggleMenu();
            ...
            <NuxtLink
              to="/videos"
              :aria-current="currentRoute === '/videos' ? 'true' : 'false'"
              class="flex flex-shrink-0 items-center gap-3 w-[90%] bg-white border-b-2 border-b-white hover:border-b-accent transition-all px-4 py-[1.5px] rounded-l-lg relative after:content-[' '] after:w-2 after:h-full after:absolute after:right-[-10%] after:rounded-l-2xl after:bg-white after:pointer-events-none aria-[current=true]:after:bg-accent"
              @click="
                () => {
                  toggleMenu();
            ...
            <NuxtLink
              to="/dashboard"
              :aria-current="currentRoute === '/dashboard' ? 'true' : 'false'"
              class="flex flex-shrink-0 items-center gap-3 w-[90%] bg-white border-b-2 border-b-white hover:border-b-accent transition-all px-4 py-[1.5px] rounded-l-lg relative after:content-[' '] after:w-2 after:h-full after:absolute after:right-[-10%] after:rounded-l-2xl after:bg-white after:pointer-events-none aria-[current=true]:after:bg-accent"
              @click="
                () => {
                  toggleMenu();
              ...
              "
              ><Icon name="icon-park-outline:data-all" /> Dashboard</NuxtLink
            >
          </nav>
          ...
          >
            <div class="w-1/2 h-full flex justify-center items-center relative">
              ...
              <nav
                ...
                class="flex items-center justify-center h-10 rounded-3xl text-base bg-accent text-secondary p-4 flex-shrink-0 gap-4"
              >
                <NuxtLink @click="toggleMenu" to="/login">Login</NuxtLink>
                <NuxtLink @click="toggleMenu" to="/sign-up">Sign Up</NuxtLink>
              </nav>
            </div>
          ...
          <div
            v-if="username"
            class="w-1/2 flex items-center justify-center mb-4"
          >
            <button
              @click="
                () => {
